...
exports.handler = async (event) => {
    ...
    const id_compra = data.id_compra;  // El ID de la reseña que se desea eliminar
    ...
    try {
        ...
        const params = {
            ...
            Key: {
                ...
                id_compra: id_compra  // Clave de ordenación
            }
        };

        const existingCompra = await dynamodb.get(params).promise();

        if (!existingCompra.Item) {
            return {
                statusCode: 404,
         ...
         // Proceder con la eliminación de la COMPRAS_TABLE
         const deleteParams = {
            TableName: COMPRAS_TABLE,
            Key: {
                user_id: user_id,
